...
function setupGui() {
  ...
  gui = new dat.GUI();
  estimatePoseController = gui.add(params, 'estimatePoseEnable')
    .name('Estimate Pose')
    .onChange( (value) => {
      poseNet.estimatePoseEnable = value;
      if (value) {
        poseNet.load().then( () => {
          console.log("Model Reloaded after Estimate Enable.");
          ready = false;
          setupTrackingAlgorithm();
        } );
      }
...
function draw() {
  ...
  if (video.pixels.length > 0) {
    if (cvReady()) {
      ...
      if (frameCount % 100 == 0) {
        // params.estimatePoseEnable = true;
        // estimatePoseController.updateDisplay();
    
        // poseNet.estimatePoseEnable = true;
        // poseNet.load().then( () => console.log("Model Reloaded after Estimate Enable.") );
        // setupTrackingAlgorithm();

        // params.estimatePoseEnable = false;
        // estimatePoseController.updateDisplay();    
        // poseNet.estimatePoseEnable = false;
      }
